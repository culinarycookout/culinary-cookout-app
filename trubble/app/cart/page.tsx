...
export default function TrubbleCartPage() {
  ...
  if (trubbleLoading) return <div className="min-h-screen bg-black text-white flex items-center justify-center">Loading cart...</div>;
      ...
      {trubbleCartItems.length === 0 ? (
        ...
        <div className="space-y-4">
          {trubbleCartItems.map((item) => (
            <div key={item.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 shadow-lg flex justify-between items-center">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => trubbleRemoveFromCart(item.id)} className="text-red-600 hover:text-red-400 text-sm font-bold underline">
                  Remove
                </button>
              </div>
            </div>
          ...
          <div className="bg-zinc-800 border border-zinc-700 rounded-xl p-6 shadow-lg mt-6">
            ...
            <button onClick={trubbleClearCart} className="w-full bg-zinc-700 hover:bg-zinc-600 text-white py-3 rounded-xl font-bold transition-all mb-3">
              Clear Cart
            </button>
          </div>
