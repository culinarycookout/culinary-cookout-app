      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 max-w-md w-full text-center shadow-xl">
          ...
          <p className="text-zinc-300 mb-6">
            Thank you, <span className="font-semibold text-white">{customerName}</span>! We&apos;ve received your order and will reach out via WhatsApp shortly.
          </p>
          <Link
            href="/"
            ...
            <div className="lg:w-[400px]">
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 sticky top-4">
                ...
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                  {/* ✅ FIXED MAPPING: Now reads the exact keys from your cart data */}
                  {cart.map((item) => {
                    const itemTotalPrice = (item['Price'] || item.price || 0) * item.quantity;

                    return (
                      <div key={item.cartInstanceId || item.id} className="flex justify-between items-start border-b border-zinc-800 pb-4">
                        <div>
                          <h3 className="font-bold text-white">{item['Item Name'] || item.name}</h3>
                          {item.SIZE && <p className="text-xs text-zinc-400">Size: {item.SIZE}</p>}
                          {/* Use item.breakdown if it exists */}
                          {item.breakdown && <p className="text-xs text-orange-400 mt-1">Details: {item.breakdown}</p>}
                          
                          <div className="flex items-center gap-3 mt-2">
                            <div className="flex items-center space-x-2">
                              <button
                                type="button"
                                onClick={() => updateQuantity(item.cartInstanceId || item.id, Math.max(1, item.quantity - 1))}
                              ...
                              <button
                                type="button"
                                onClick={() => updateQuantity(item.cartInstanceId || item.id, item.quantity + 1)}
